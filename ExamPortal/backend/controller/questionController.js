...
const answerObject = (body,headers,weightage)=>{
...
const saveCandidateAnswers = async(req,res)=>{
    ...
    if(checkAnswer.ans === req.body.checkedOption){
        if(existingAnswer === null){
            let answerDetail = answerObject(req.body, req.headers, checkAnswer.weightage)
            ...
            let updateScoreStatus = await test.findOneAndUpdate(
                ...
                {   
                    $push: {answers:{answerSubmitted: req.body.checkedOption,questionId: req.body.qId}},
        ...
        if(existingAnswer === null){
            let answerDetail = answerObject(req.body,req.headers,0)
            await answerDetail.save()
        }
        else{
